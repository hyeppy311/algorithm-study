...
# 짧은 리스트는 변수 설정 안해도 됨 (오른손, 왼손 지정된 숫자)
# 키패드를 2차원으로 선언 후 어떻게 좌표를 저장할 것인가? 아니면 숫자를 좌표로 구현하고 어떻게 계싼할 것인가? 
# 2,5,8,0의 경우에서 어떻게 구현할 것인가? 
# 다음으로 옮겨갈 숫자에서 현재 오른손과 왼손의 위치를 각각 빼준다.
# 비교하여 더 합이 더 적은 수의 손을 변수에 저장
# 같을 경우 입력으로 들어온 손으로 저장 

# 리스트의 값을 문자열, 숫자 둘 중 하나로 통일 시켜야함 
# 처음 키패드를 선언할때 숫자는 숫자 그대로 문자는 문자 그대로 했음 
