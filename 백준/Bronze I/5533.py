...
# 입력으로 받은 값을 열로 어떻게 묶을 것인가?   각 들어온 값을 3개의 리스트에 나누거나, zip() 이용
# 같은 값을 어떻게 비교 할 것인가?           count() 이용
# 값이 없을 경우 어떤 변수에 저장할 것인가?     score = 0, += 해주기
# 참가자 변수를 어떻게 설정할 것인가?         [0]인 리스트를 참가자 수 만큼 생성 

# 게임별로 묶어서 중복 없는 점수를 더해주기 
